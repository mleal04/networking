...
void modify_buffer(char *buffer, char *dl)
{
    char *file_name_pos = strstr(buffer, "File-Name: ");
    if (file_name_pos)
    {
        file_name_pos += strlen("File-Name: "); // Move pointer to the actual file name

        // Find the end of the file name (line break)
        char *end_pos = strchr(file_name_pos, '\n');
        if (end_pos)
        {
            char new_buffer[BUFSIZ]; // Ensure buffer is large enough
            char modified_name[256];

            // Modify the file name (example: prepend "modified_")
            snprintf(modified_name, sizeof(modified_name), "./%s/%.*s", dl, (int)(end_pos - file_name_pos), file_name_pos);

            // Rebuild the buffer with the modified file name
            snprintf(new_buffer, sizeof(new_buffer), "%.*s %s%s",
                     (int)(file_name_pos - buffer), buffer, modified_name, end_pos);

            // Copy back to the original buffer
            strncpy(buffer, new_buffer, BUFSIZ - 1);
            buffer[BUFSIZ - 1] = '\0'; // Ensure null termination
        }
    }
}
...
{
    // must take in: ./name , node IP, node PORT , dl or quit , quiet
    if (argc < 4)
    {
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else if (strncmp(request, "GET", 3) == 0)
            {
                ...
                if (quiet_flag == 0 || strcmp(status_code, "400") == 0)
                {
                    modify_buffer(buffer, argv[3]);
                    printf("%s\n", buffer);
                }
                else
                { // quiet --> not need to fix the buffer
                    continue;
                }
            }
            ...
            else if (strncmp(request, "FINDR", 5) == 0)
            {
                // beware of quiet
                printf("%s\n", buffer);
            }
            else if (strncmp(request, "GETR", 4) == 0)
            {
                // quiet or not
                if (quiet_flag == 0 || strcmp(status_code, "400") == 0)
                {
